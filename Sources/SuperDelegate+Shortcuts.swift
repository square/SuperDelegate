...
public protocol ShortcutCapable: ApplicationLaunched {
    ...
    /// Called when your app receives a shortcut item. Will not be called for shortcuts that were delivered to the app via loadInterfaceWithLaunchItem(_:). Guaranteed not to be called with a shortcut item your app can not handle. Execute completionHandler when your application has handled the shortcut.
...
extension SuperDelegate {
    ...
    @available(iOS 9.0, *)
    final public func application(application: UIApplication, performActionForShortcutItem shortcutItem: UIApplicationShortcutItem, completionHandler: (Bool) -> Void) {
        ...
        let canHandleShortcutItem = shortcutCapableSelf.canHandleShortcutItem(shortcutItem)
        
        if canHandleShortcutItem {
            shortcutCapableSelf.handleShortcutItem(shortcutItem) {
